...
async function extractHighlightsFromOpList(opList) {
  ...
  for (let i = 0; i < opList.fnArray.length; i++) {
    ...
    if (op === OPS.save) {
      ...
      // Determine if RGB represent a highly saturated highlight
      ...
      if (isBrightSaturated && currentPathPoints.length > 0) {
...
async function runStructuredLayoutParser(parser, doc, numPages, pdfName, template, answersBuffer = null) {
  ...
  for (let pageNum = 1; pageNum <= numPages; pageNum++) {
    ...
    // Retrieve vector graphics highlights (Layer 3)
    let highlights = [];
    try {
      const opList = await page.getOperatorList();
      highlights = await extractHighlightsFromOpList(opList);
    } catch (err) {
      console.warn(`Failed to retrieve operator list on page ${pageNum}:`, err);
    }

    // Group items into lines
    const pageLines = groupItemsIntoLines(textContent, annotations, highlights).map(line => ({
      ...line,
      pageNum
    }));
...
function groupItemsIntoLines(textContent, annotations, highlights = []) {
...
function parseQuestionsFromLines(pageLines, pageNum) {
  ...
  for (let i = 0; i < pageLines.length; i++) {
    ...
    // Generic Block Option matching with punctuation
    let optMatch = null;
    const punctMatch = line.text.match(/^\s*\(?([A-Ea-e])\)?\s*[\.\:\)\-]\s*(.+)/);
    if (punctMatch) {
      const letter = punctMatch[1].toUpperCase();
      const text = punctMatch[2].trim();
      const idx = letter.charCodeAt(0) - 65;

      // Ensure sequential consistency
      if (currentQ && (idx === currentQ.options.length || (idx === 0 && currentQ.options.length === 0))) {
        optMatch = [line.text, letter, text];
      }
    }

    // Space-only Option matching
    if (!optMatch && currentQ) {
      const spaceMatch = line.text.match(/^\s*([A-Ea-e])\s+([A-Za-z0-9_].*)/);
      if (spaceMatch) {
        const letter = spaceMatch[1].toUpperCase();
...
const DETECTORS = [
  {
    ...
    detect: (q, options, features, context) => {
      if (context.separateAnswersMap && context.separateAnswersMap[q.num] !== undefined) {
        return { detectedAnswer: context.separateAnswersMap[q.num], confidence: 98 };
      }
      return null;
    }
  },
  {
    ...
    detect: (q, options, features, context) => {
      if (context.endAnswersMap && context.endAnswersMap[q.num] !== undefined) {
        return { detectedAnswer: context.endAnswersMap[q.num], confidence: 95 };
      }
      return null;
    }
  },
  {
    ...
    detect: (q, options, features, context) => {
      ...
      if (matches.length === 1) {
        return { detectedAnswer: matches[0], confidence: 95 };
      }
      return null;
    }
  },
      ...
      if (q.explanation) {
        ...
        if (textMatch) {
          ...
          if (idx >= 0 && idx < options.length) {
            return { detectedAnswer: idx, confidence: 95 };
          }
        }
      }
      return null;
  ...
  },
  {
    ...
    detect: (q, options, features, context) => {
      ...
      if (matches.length === 1) {
        return { detectedAnswer: matches[0], confidence: 92 };
      }
      return null;
    }
  },
  {
    ...
    detect: (q, options, features, context) => {
      ...
      if (matches.length === 1 && activeOptsCount > 2) {
        return { detectedAnswer: matches[0], confidence: 90 };
      }
      return null;
    }
  },
  ...
  for (const q of questions) {
    ...
    if (template && template.answerPattern) {
      ...
      if (targetDetector) {
        ...
        if (result) {
          detectedIndex = result.detectedAnswer;
          confidence = 98;
