...
export async function sendEmail({ to, subject, text, html }) {
  console.log(`\n[EMAIL SERVICE] Initializing email generation...`);
  console.log(`[EMAIL SERVICE] Target Recipient: ${to}`);
  console.log(`[EMAIL SERVICE] Subject: ${subject}`);
  console.log(`[EMAIL SERVICE] Sender Profile: "${senderName}" <${senderEmail}>`);

  const activeApiKey = apiKey || (pass && pass.startsWith("xkeysib-") ? pass : null);

  // If we have a Brevo API key, use Brevo's direct HTTP API.
  if (activeApiKey) {
    console.log(`[EMAIL SERVICE] Selected Delivery Channel: Brevo HTTP API (Key verified)`);
    try {
      ...
      console.log(`[EMAIL SERVICE] Email Sent Successfully | Message ID: ${responseData.messageId}\n`);
      return { success: true, messageId: responseData.messageId };
    } catch (error) {
      console.error(`[EMAIL SERVICE] Email Failed | Reason: ${error.message}\n`);
      return { success: false, error: error.message };
    }
  }

  console.log(`[EMAIL SERVICE] Selected Delivery Channel: Brevo SMTP Relay`);
  if (!transporter) {
    ...
    console.log("------------------------------------------------------------------------------");
    console.log(`[EMAIL SERVICE] Email Sent Successfully | Mock Logged to Console\n`);
    ...
    <head>
      ...
      <style>
        ...
        .header {
          ...
          text-align: center;
          color: #ffffff;
          border-bottom: 3px solid #2E76C0;
        }
        .header-emoji {
        ...
        .btn {
          ...
          font-weight: 700;
          font-size: 14px;
          box-shadow: 0 4px 12px rgba(46, 118, 192, 0.15);
        }
        .footer {
          background-color: #f8fafc;
        ...
        <tr>
          <td>
            <div class="container">
              ...
              <!-- Footer -->
              <div class="footer">
                <p>MedAssess Pro &copy; 2026. All rights reserved.<br>Empowering next-generation clinical assessment and NEET PG excellence.</p>
              </div>
            </div>
          </td>
        </tr>
...
export async function sendStudentWelcomeEmail({ name, email, password, loginUrl }) {
  const subject = "Welcome to MedAssess Pro - Your Login Credentials";
  ...
  const text = `Hello ${name},\n\nYour account has been created on MedAssess Pro.\n\nLogin credentials:\nEmail: ${email}\nPassword: ${password}\n\nLogin here: ${loginUrl}`;
...
export async function sendAdminWelcomeEmail({ name, email, password, loginUrl, permissions = [] }) {
  const subject = "Welcome to the MedAssess Team - Administrative Access Ready";
  ...
  const permListHtml = permissions.map(p => `
    <li style="margin-bottom: 6px; text-transform: capitalize; font-size: 13px;">
      <span style="color: #2E76C0; font-weight: bold; margin-right: 6px;">✓</span> <strong>${p.replace('_', ' ')}</strong>
    </li>
  `).join('');

  const bodyHtml = `
    <p>Hello <strong>${name}</strong>,</p>
    <p>You have been registered as an Administrator on MedAssess Pro with security roles to manage features and content.</p>
  ...
  const bodyHtml = `
    ...
    <div class="card">
      ...
      <div class="card-row">
        ...
        <span class="card-value" style="font-family: inherit; font-weight: bold; color: #2E76C0;">${examName}</span>
      </div>
    ...
    <div class="card" style="border-left: 4px solid ${statusColor};">
      ...
      <div style="display: flex; gap: 20px; flex-wrap: wrap;">
        ...
        <div style="flex: 1; min-width: 100px;">
          ...
          <span class="card-value" style="font-weight: bold; font-size: 16px; color: #2E76C0;">${accuracy}%</span>
        </div>
        <div style="flex: 1; min-width: 100px;">
    ...
    <div class="card" style="background-color: #f8fafc; border: 1px solid #e2e8f0;">
      <table style="width: 100%; border-collapse: collapse; font-size: 14px;">
        ...
        <tr>
          ...
          <td style="padding: 6px 0; color: #2E76C0; font-weight: bold;">${examName}</td>
        </tr>
        <tr>
