...
async function initializeParser() {
  ...
  try {
    try {
      ...
      console.warn("Failed to resolve nested @napi-rs/canvas package, falling back to root resolution.", e.message);
    ...
    console.warn("CRITICAL WARNING: Failed to load native @napi-rs/canvas package. Screenshot and OCR capabilities will be disabled. Fallback to mock polyfills. Error:", err.message);

    // Provide simple mocks so pdf-parse doesn't throw ReferenceError when parsing text-based PDFs
  ...
  try {
    ...
    console.error("Failed to load pdf-parse package:", err);
    throw err;
  }
  isInitialized = true;
}
...
async function runStructuredLayoutParser(parser, doc, numPages, pdfName, template, answersBuffer = null) {
  ...
  // For large PDFs, we skip image extraction entirely because @napi-rs/canvas can leak memory and crash the process.
  if (numPages <= 30) {
    ...
    for (const pageNum of pagesWithQuestions) {
      try {
        ...
        console.warn(`Aborting further image extraction to prevent memory crash.`);
        break;
      }
    }
  } else {
    console.log(`Skipping image extraction because PDF has ${numPages} pages (max 30 allowed to prevent memory crash).`);
  }
...
function matchOptionLine(text) {
  const regexes = [
    ...
    /^\s*([A-Ea-e])\s*[\.\:\)\-]\s*(.*)/     // Match e.g. "A. option text", "A) option text", "A: option text", "A - option text"
  ];
...
function splitInlineOptions(text) {
  const delimiters = [
    { regex: /\s+([B-Eb-e])\s*[\.\:\)\-]\s+/g, start: /^\s*\(?([A-Ea-e])\)?\s*[\.\:\)\-]\s*(.*)/ },
...
function parseQuestionsFromLines(pageLines, pageNum) {
  ...
  const questionRegex = /^(?:(?:Q|q)(?:uestion)?(?:s)?)?\.?\s*(\d+)(?:\s*[\.\:\)]\s*(.*)|\s+(["“'‘]?[A-Z].*))/;
  ...
  for (let i = 0; i < pageLines.length; i++) {
    ...
    if (currentQ) {
      ...
      if (match) {
        ...
        // Relaxed sequential consistency: allow options even if one was skipped or misread
        if (idx >= 0 && idx <= currentQ.options.length + 1) {
          optMatch = match;
        }
      }
    }
...
async function evaluateCorrectAnswers(questions, pdfName, template = null, context = {}) {
  ...
  for (let i = 0; i < questions.length; i += batchSize) {
    ...
    const batchEvaluated = await Promise.all(batch.map(async (q) => {
      ...
      if (template && template.answerPattern && template.answerPattern !== "mixed_formats") {
      ...
      // Gemini AI Fallback has been removed as per user request.
      // Questions without detected answers will simply be routed to the Review Queue (confidence = 40)
      // where the user can manually train the wizard or select answers.
